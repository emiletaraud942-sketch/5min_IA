...
export function GoogleSignInButton({ label = "Continuer avec Google" }: { label?: string }) {
  ...
  async function handleClick() {
    ...
    if (error) {
      setError("Connexion Google indisponible pour l'instant. Réessaie ou utilise l'email.");
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        ...
        onClick={handleClick}
        disabled={loading}
        className="inline-flex w-full items-center justify-center gap-2.5 rounded-full border border-sand-300 bg-white px-5 py-3 text-sm font-semibold text-brand-900 transition-colors hover:bg-sand-50 disabled:cursor-not-allowed disabled:opacity-50"
      >
        <GoogleLogo />
